...
function complete_statement(
  ...
  {
    const indent_position: number = current_line.text.lastIndexOf(' '.repeat(tab_stop));
  ...
  if (current_line.text.trim() === '}') {
    ...
    if (current_line.text.endsWith('{')) {
      ...
      let braces: string;
      const allman: boolean = vscode.workspace.getConfiguration('complete-statement').get('allman', false);
      if (allman) {
        braces = `\n${less_indent_spaces}{\n${indent_spaces}` + `\n${less_indent_spaces}}`;
